...
const API_URL = "http://localhost:8080/api/container/all";
const API_URL_POST = "http://localhost:8080/api/container/create_container";
...
const App = () => {
  ...
  const deleteContainer = async (event) => {
    if (window.confirm(`Are you sure you want to delete Container ID: "${deleteId}"?`)) {
      try {
        await fetch(`http://localhost:8080/api/container/delete/${deleteId}`, {
          method: "DELETE"
        });
      } catch (e) {
        console.error("Failed to fetch containers:", e);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-1 bg-white p-6 rounded-3xl shadow-2xl border-2 border-blue-200">
            ...
            <form onSubmit={deleteContainer} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-600">
                  ID
                </label>
